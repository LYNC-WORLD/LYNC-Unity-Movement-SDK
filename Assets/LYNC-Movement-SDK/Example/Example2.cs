    ...
    private async void LyncReady(LyncManager Lync)
    {
        ...
        sendTransaction.onClick.AddListener(async () =>
        {
            ...
            if (txResult.success)
            {
                ...
                entry.callback.AddListener((eventData) => { Application.OpenURL("https://blue.explorer.movementlabs.xyz/txn/" + txResult.hash + "?network=" + LyncManager.Instance.Network.ToString()); });
